...
def load_symbol_config(symbol: str) -> dict:
    ...
    return {
        ...
        "lookback_bars": 600, "vp_va_pct": 0.70
    }


def fetch_cryptocompare_klines(symbol: str = "ETH/USDT", limit: int = 360) -> pd.DataFrame:
    try:
...
def fetch_klines(symbol: str = "BNB/USDT", interval: str = "1h", limit: int = 360) -> pd.DataFrame:
...
def compute_volume_profile(
    ...
    """Computes POC, VAH, and VAL exactly aligned with Pine Script VP ☼☾[cite: 31]."""
...
def calculate_volume_profile_gaps(df: pd.DataFrame, num_bins: int = 100, lookback_bars: int = 600, detection_pct: float = 0.07) -> list:
    """Identifies Volume Profile Gaps matching Pine Script vgSH logic[cite: 31]."""
    if df.empty or 'volume' not in df.columns or 'high' not in df.columns or 'low' not in df.columns:
        return []
