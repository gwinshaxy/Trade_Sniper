...
def fetch_klines(symbol: str = "BNB/USDT", interval: str = "1h", limit: int = 1000, timeframe: str = None) -> pd.DataFrame:
    ...
    bybit_interval = bybit_interval_map.get(actual_interval, "60")

    urls = [
        f"https://api.binance.com/api/v3/klines?symbol={binance_symbol}&interval={actual_interval}&limit={min(limit, 1000)}",
        f"https://api.binance.us/api/v3/klines?symbol={binance_symbol}&interval={actual_interval}&limit={min(limit, 1000)}",
...
def evaluate_signals(df: pd.DataFrame, symbol: str = "ETH/USDT", account_balance: float = 10000.0, **kwargs) -> dict:
    ...
    disable_htf = bool(kwargs.get("disable_htf", config.get("disable_htf", False)))
